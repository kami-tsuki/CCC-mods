    ...
    fun annotate(text: String, docs: Map<String, String>, header: List<String> = emptyList()): String {
        ...
        return buildString {
            ...
            text.lines().forEach { line ->
                key.matchEntire(line)?.destructured?.let { (indent, name, rest) ->
                    while (parents.isNotEmpty() && parents.last().first >= indent.length) parents.removeLast()
                    doc(docs, parents.map { it.second } + name)?.takeIf(seen::add)?.let { append(indent).append("// ").append(docs[it]).append('\n') }
